...
*Task 3: Streamline the `pop` dataframe using `select` so that we only retain the `fips` coloumn and all columns that start with "pop2".*

```{r streamlinePop, exercise=TRUE, exercise.setup="inspectPop-setup"}
...
```{r gatherPop-solution}
pop %<>% gather(starts_with("pop2"),key="year",value="size") %>% na.omit
```


*Task 5: Use the `gather` function to collect columns starting "pop2" into two columns, where the `key` is "year" and the `value` is size. As a final part of the pipeline, remove rows with `NA` values.*
