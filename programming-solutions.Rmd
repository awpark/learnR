...
This exercise is about writing *scripts*, purpose-built computer programs for performing some analysis. Our scripts will be written using the *RStudio Editor* and compiled using Rstudio. In this exercise we review many basic numerical operations and R functions, programming style, the development of custom functions, pipes, flow of control and loops. If you're relatively new to programming, you may find it helpful to first review this short video.
...
As you learn more tidy R methods, you'll find you rely on loops less. However, they can still be useful when we are iterating our data in a repetitive way. Here's a simple example where we loop through each year (1999-2007) and count how many states had at least one case of WNV. Run the code to see what it does.
...
Using the example above as a guide, write a loop that prints the number of fatal cases each year
...
You can check this function works by creating an object (say, x) that is the return of the function `sevRegRatio` when you supply data frame "wnv". Check this works by creating x and inspecting it using the `head` function. 
...
You'll see that some entries have "Inf" for the ratio. This stands for infinity and is caused when all $n$ cases are severe, because the ratio becomes $n/0$ which is infinity. In the code chunk below, paste the function and the creation/inspection of x (i.e. paste the contents of the two code chunks above into the empty one below). Now try to edit the function so that it won't write out the rows where the ratio is infinity. You can achieve this using an `if` statement. Remember, you can get help using the solution button. Good luck and great job!
